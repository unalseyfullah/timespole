...
/* Seyfullah Hepsi yazısının dil düzenlemesi başlangıç */
foreach ($categories as $cat) {
	if($cat->slug == "gida"){
		$cat_all = "Tümü";
	} elseif($cat->slug == "category-1"){
		$cat_all = "All";
	} elseif($cat->slug == "kategorie-1"){
		$cat_all = "Alle";
	} 
}
/* Seyfullah Hepsi yazısının dil düzenlemesi bitiş */
?>
	...
	<div id="<?php echo esc_attr( $works_id ); ?>" class="stm_works_wr<?php echo esc_attr( $css_class ); ?>">
		<?php if( $style == 'grid_with_filter' && $categories ): ?>
			<ul class="works_filter">
				<li class="active"><a href="#all"><?php esc_html_e( $cat_all, 'consulting' ); ?></a></li>
